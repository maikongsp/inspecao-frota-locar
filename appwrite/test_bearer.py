import urllib.request
import json

KEY = "standard_"
PROJ = "6aaf35a1003b73ac8b04"
DB_ID = "locar_betim_db"
HOST = "https://nyc.cloud.appwrite.io/v1"

# Vamos listar todas as coleções ou testar com role admin
url = f"{HOST}/databases/{DB_ID}/collections"
payload = {
    "collectionId": "inspections",
    "name": "Inspecoes e Laudos",
    "permissions": ["read(\"any\")", "create(\"any\")"]
}

# Testa passando como Bearer Token
req = urllib.request.Request(url, data=json.dumps(payload).encode("utf-8"), headers={
    "User-Agent": "Mozilla/5.0",
    "X-Appwrite-Project": PROJ,
    "Authorization": f"Bearer {KEY}",
    "Content-Type": "application/json"
})
try:
    with urllib.request.urlopen(req) as resp:
        print("Sucesso via Bearer:", resp.read().decode("utf-8"))
except Exception as e:
    err = getattr(e, "read", lambda: b"")().decode("utf-8")
    print("Erro via Bearer:", e, err)
